...
| `import` | Imports modeled target-visible roots and supported standalone MCP config rows into manifest-owned declarations when ownership can be reviewed; import itself creates no host ownership. The current CLI does not yet author extension declarations. Exact extension import is planned for Codex, Claude Code, OpenCode, and Pi, while Antigravity lacks recoverable source provenance. External carrier adoption remains a separate `apply --manage-existing` path for an authored and locked declaration. |
