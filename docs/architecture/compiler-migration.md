...
| ARCH-G14 | guarantee | The operation plan describes semantic obligations; the State Barrier alone lowers them with selected paths and platform facts into physical StateDir/RecoveryDir capacity and consumable authority. | User decision; `ARCHITECTURE.md` and current lifecycle contracts. | Apply and Refresh call `recoverygate.ReserveForwardEffectStructure` with the owner schedule and transitional legacy path/count binding. The State Barrier performs branch-aware physical lowering through the same `StateDirAuthority` planning formulas, proves the legacy reservation dominates that work, and still issues the legacy scalar authority pending cursor cutover. Direct `ReserveForwardEffects` use is fixture-only. | Behavioral parity. | effect-envelope phase/state-barrier phase. | Physical resource cost cannot be derived without moving operation semantics into the barrier owner. |
...
| closeout phase | every prior artifact | provisional: surface compilation and production-consumer cutover, exact operation authority/fingerprint/revision-role compilation, pure workflow lowering, and named State Barrier verification are present. The count-only effect-envelope seed and exact-path blocking classification still prevent closeout. | any duplicate source, bypass, parity drift, or unowned exception remains |
