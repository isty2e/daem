...
| ARCH-G17 | guarantee | Workflows gather boundary evidence, invoke pure owners, acquire capabilities, sequence effects, and project results; they do not retain competing surface, authority, fingerprint, or capacity grammars. | User decision; `ARCHITECTURE.md`. | MCP production callers, read-only location inventory, manifest location selection, and selected apply/refresh/recover callers invoke `catalog.Product`, `operationplan`, and `recoverygate`. Instruction, Skill, Hook, HookAsset, and Extension views compile from owner-local snapshots; remaining adopt/diagnose/lock/readiness/effect consumers, workflow-local fingerprint DTOs, and several mutating-operation authority grammars remain migration work rather than accepted end state. | Behavioral parity. | surface-cutover through operation/effect closeout remains active. | A use case owns a genuinely operation-specific semantic contract not representable by the shared compiler. |
| ARCH-G18 | guarantee | Old implementation remains authoritative during shadow comparison. A caller flips only after exact required parity, and the old source is deleted after its last consumer moves. | User decision; architecture change rules. | Bounded MCP joins, read-only location inventory, manifest location selection, apply/refresh/recover authority subprojections, and the exact refresh and recovery operation fingerprints have flipped. Remaining non-MCP consumers, apply and other mutating-operation compilers, and ordered effect settlement remain under the shadow-then-delete rule; final closeout has not occurred. | Exact or behavioral as classified below. | surface-schema phase through closeout phase. | A separately authorized migration requires staged dual-version support. |
...
| Non-MCP host-surface views | `internal/hostsurface/catalog`: managed-path, Hook/HookAsset, and Extension owner-parity, target/scope lookup, default/exact-root selection, and negative-seed tests; `internal/realization/profile`: managed-path relative-path, delegated-route/order, and HookAsset snapshot tests; `internal/topology/extension`: carrier namespace coverage; `internal/workflow/list`: exact location inventory, manifest selection, and output tests | exact Instruction/Skill placement, admission/default, discovery/runtime, and route facets; Hook aggregate placement/codec/routes; shared HookAsset physical placement; Extension carrier/source/namespace/delegated-route/order facets; stable owner/placement order and SurfaceID ordering; defensive copies; list inventory and manifest-selection parity | read-only list inventory and manifest selection consume compiled views. Authored relative-path interpretation remains with Realization. Adopt/diagnose/lock/readiness/effect consumers remain on owner-local projections until bounded differential cutover. Skill compatibility policy remains a separate Supply oracle. |
...
| surface-cutover phase | surface-schema phase views and consumer census | bounded cutover, deletion map, one source per migrated facet. MCP waves 1–4 use compiled target/scope, probe/provider, adopt-local extractor, subject, readiness/observe, and owner-order views while preserving Pi importability and realization/lock ownership. Non-MCP waves 1–2 move read-only list inventory and manifest location selection for managed paths, Hook/HookAsset, and Extension routes while authored relative-path interpretation remains Realization-owned. Remaining caller groups each require differential parity before their old join is removed. | any consumer requires different semantics or facade becomes authority |
...
| closeout phase | every prior artifact | provisional: non-MCP compilation, list inventory and manifest-selection cutovers, exact refresh/recovery fingerprints, and named State Barrier verification are present, but remaining production surface joins, workflow-local operation/fingerprint compilers, the count-only effect-envelope seed, and exact-path blocking classification still prevent closeout | any duplicate source, bypass, parity drift, or unowned exception remains |
...
list-inventory and manifest-selection cutovers, apply/refresh/recover authority,
exact refresh/recovery fingerprints, reservation-demand seeds, State Barrier
ownership and named verification, readiness middle-end, and report-only
compiler-shadow guards. This is not final program closeout. Remaining
Instruction, Skill, Hook, HookAsset, and Extension consumer joins;
mutating-operation compilers and fingerprint serializers; ordered effect
settlement; and semantic guard replacement remain required work.
