...
| ARCH-G12 | guarantee | Every effectful operation has a typed, ordered effect envelope whose complete demand is admitted before its first external or visibility effect. | User decision; `ARCHITECTURE.md` transaction/recovery contract. | `operationplan` provides the passive immutable effect-structure algebra and legacy demand projection; `EffectStepTerminal` globally ends the selected structure, while `EffectStepNoOp` is the only continuing no-effect branch; Effect and Refresh own schedules. Apply compiles one canonical full/final structure with ordinal byte-neutral references, one operation-level Statefile binding, exact legacy-demand parity, and exact provider-suffix equality at both replans. Its outer settlement order is the Effect-owned Apply segment, global claim retirement, carrier removal, final host routes, locked relation-order classes, delegates, and terminal-last global adoption, with explicit no-change, continuation, and fail-fast branches. Apply now passes the exact prepared Effect-owned core plan through both payload-free and payload-bearing workflow branches and consumes its immutable structure with one operation-local cursor across journal publication and retirement, ownership preparation, provisional promotion and finalization, managed and aggregate observation/mutation, Statefile publication, acceptance, and final validation checkpoints. The existing shared visibility-gate protocol still owns before-effect and after-effect authority callbacks, while rollback/compensation and outer global-retirement/carrier/route/order/delegate/adoption cursor settlement remain deferred. Both Apply and Refresh hand their full structure to the State Barrier. Refresh consumes the exact reserved structure through an operation-local cursor for barrier, external-attempt, observation, StateDir, descendant binding/validation/publication, persistence, failure-terminal, and success-terminal checkpoints. The cursor owns and drives the reserved physical StateDir and descendant authorities directly. Refresh production reservation lowers every cursor-reachable alternative and admits their deterministic componentwise physical frontier, while supplying the selected Statefile path separately to the State Barrier; the flat `CompileRefresh` demand remains only a differential test oracle. Cleanup-only Recovery additionally compiles its exact journal-retirement schedule from the selected cleanup authority and consumes last-safe-point validation, cleanup-authority/layout checks, optional phase advance and residue removal, control retirement, garbage cleanup, retained-authority close, and terminal failure branches through an operation-local cursor; Journal retains transition meaning and the existing RecoveryDir-only physical budget remains authoritative. Active Recovery now also compiles and consumes a fixed-size, zero-demand runtime cursor for the post-cleanup `ExecuteActive` tail: plan and prepared-layout validation, active-entry identity, prepared-control present-versus-publish choice, active-record validation, active-to-residue retirement, phase and finalizing layout checks, record advance, residue cleanup, control retirement, garbage cleanup, and closed terminal prefixes. Journal retains transition meaning, and the cursor grants no filesystem authority. Active-Recovery removal cleanup additionally compiles one compact zero-demand repeated structure over the complete bounded intent set; `effect/execute` consumes effect-time observation, the closed confirm-absence/promote-residue/cleanup-progress choice, action settlement, discharge, and final retirement-readiness through one operation-local cursor. Complete batch preflight, physical cleanup reservations, partial-effect recovery, and Journal retirement meaning remain authoritative in their existing owners. Active Recovery additionally compiles and consumes a classification- and caller-specific zero-demand outer-settlement structure bound to exact `Plan.SameExecutionAuthority`: it spans preparation, clean/finalize/rollback branches, compact ordered host visits, claim transitions, post-effect reclassification, the existing removal-cleanup and active-retirement cursor handoffs, outer visibility/currentness checks, and terminal authority settlement. `effect/execute` retains rollback, compensation, claim, and outcome meaning; standalone Recovery closes owned mutation authority exactly once and joins close failures, while guarded Apply rollback borrows authority. Existing Recovery semantic and physical budgets remain authoritative. Apply additionally consumes a separate constant-sized, zero-State-Barrier-demand failure-settlement structure: typed forward-failure dispositions permit only claim-preparation rollback, prepared-effect rollback, guarded active-Recovery handoff, or explicit no-compensation completion. Ownership rollback and journal retirement remain behind the compensation visibility protocol, guarded host rollback remains Active Recovery-owned, and primary errors retain retirement and mutation-authority close failures. Outer post-effect continuation cursor consumption and remaining flat/scalar settlement removal remain pending. | Exact refusal timing and effect ordering. | effect-envelope phase remains active. | Supported dynamic work cannot be bounded before effects and the product contract must be reduced or separately changed. |
| ARCH-G13 | guarantee | Exclusive branches reserve the maximum reachable demand rather than the sum; sequential and repeated obligations preserve exact bounded demand and cannot add work after effects begin. | User decision; `ARCHITECTURE.md`. | `operationplan` derives a bounded deterministic nondominated frontier of cursor-reachable semantic demands, preserving terminal prefixes separately until later sequence, repetition, choice, trigger, and conditional work has been excluded. `recoverygate` lowers each alternative with the selected StateDir state and descendant path, then takes the maximum physical path/entry/byte work. The resulting branch-aware lower is a mandatory shadow oracle. Apply still receives the dominating legacy scalar authority; Refresh reserves exactly the componentwise physical frontier of cursor-reachable alternatives, without combining mutually exclusive semantic counts, duplicate legacy semantic counters, or a production flat-demand input. | Behavioral parity and pre-effect refusal parity. | effect-envelope phase. | Runtime evidence proves branches are not closed/exclusive or promotion can create unplanned work. |
...
| Recovery authority/fingerprint | `internal/operationplan`; recover execution/authority/replacement/cleanup tests including `TestCompiledActiveRecoveryFingerprintMatchesLegacyProjection`, `TestCompiledCleanupRecoveryFingerprintMatchesLegacyProjection`, active-journal retirement-structure ordering/failure-prefix tests, and active-Recovery outer-settlement authority/close/rollback regressions | active and cleanup authority, drift, terminal classification, byte-identical active and cleanup-only operation projections, pure logical/physical domain-step compilation, a deterministic zero-demand post-cleanup active-journal transition tail, and exact caller-specific active-Recovery outer settlement bound to `Plan.SameExecutionAuthority`; recover authority DTO omits Family/Containment | active and cleanup-only operation fingerprints compile in `operationplan`; workflow lowers paths while journal-owned action, statefile, cleanup-obligation, claim-transition, and retirement semantics remain owner-supplied or Effect-owned inputs. The outer cursor grants no physical authority and preserves owned-versus-borrowed authority lifecycle. |
...
| Effect structure and forward reservation | `internal/operationplan` envelope/demand/cursor tests including terminal sequence, terminal handoff, repetition, trigger, no-op, and demand-frontier regressions; recoverygate reservation and high-cardinality tests; apply reservation, prepared-plan, failure-settlement, indeterminate-outcome, ownership-rollback, retirement-failure, and authority-close regressions; Refresh, journal retirement, active-Recovery removal-cleanup, and active-Recovery outer-settlement cursor tests | pre-effect atomic reservation and exact consumption; terminal branches authorize no later obligation, no-op branches continue, reachable demand alternatives stop at terminal prefixes, and legacy demand remains a conservative differential bound; Apply forward failures select only their typed settlement branch while preserving journal-retention and primary/secondary error contracts; active Recovery preserves deterministic host ordering, branch settlement, and exactly-once owned-authority close without giving the cursor outcome authority | physical path/entry/byte/census lowering stays in recoverygate `StateDirAuthority.ReserveOperation`; owner packages retain outcome and transition meaning. |
...
| effect-envelope phase | operation-authority phase facts and operation closure | passive typed structure, closed choices/triggers/conditionals, bounded demand frontiers, an exact Apply full/final structural schedule with ordinal byte-neutral references, one Statefile binding, provider-suffix equality, and production-matching outer settlement order, Apply/Refresh structural State Barrier lowering, a Refresh exact-schedule runtime cursor with structural physical reservation, a cleanup-only Recovery cursor pilot that consumes the exact journal-retirement and retained-authority-close schedule without moving journal semantics or changing RecoveryDir physical budgets, and active-Recovery cursor pilots covering compact removal cleanup, fixed post-cleanup retirement transitions, and exact outer settlement across clean/finalize/rollback branches, host visits, claims, reclassification, cursor handoffs, visibility checks, compensation, and owned-versus-borrowed authority closure. Existing Recovery semantic/physical budgets remain authoritative. Apply forward failures additionally consume a constant-sized zero-demand settlement structure for claim-preparation rollback, prepared-effect rollback, guarded active-Recovery handoff, and explicit no-compensation completion, preserving compensation visibility gates, journal retention, and error precedence. Still required: Apply post-effect continuation cursor consumption, branch-selected Apply reservation reduction, and removal of remaining flat demand and scalar runtime counters. | work cannot be bounded before effects or branch algebra is not closed |
...
| closeout phase | every prior artifact | provisional: surface compilation and production-consumer cutover, exact operation authority/fingerprint/revision-role compilation, pure workflow lowering, named State Barrier verification, Refresh cursor settlement, cleanup-only Recovery cursor settlement, and active-Recovery removal-cleanup, retirement-tail, and outer-settlement cursor pilots are present. Incomplete Apply post-effect continuation cursor settlement, remaining flat/scalar effect authority, and exact-path blocking classification still prevent closeout. | any duplicate source, bypass, parity drift, or unowned exception remains |
