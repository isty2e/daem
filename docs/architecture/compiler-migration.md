...
| lock mutation | `transaction.CaptureStateDirAuthority` | lockfile write | Preserve lock compatibility while using canonical StateDir authority. Do not add RecoveryDir journal domains. |
...
| recoverygate itself | `transaction.StateDirAuthority` plus compiled `operationplan.Demand` | joint barrier and reservation | Becomes the logical State Barrier owner; lower mechanics stay separate. |
