...
| ARCH-G12 | guarantee | Every effectful operation has a typed, ordered effect envelope whose complete demand is admitted before its first external or visibility effect. | User decision; `ARCHITECTURE.md` transaction/recovery contract. | `operationplan` provides the passive immutable effect-structure algebra and legacy demand projection; Effect and Refresh own schedule shadows. Apply now composes one structural reservation shadow across provider and final continuations, proves exact legacy-demand parity before reservation, and rejects a post-provider final suffix that differs from the reserved structure. Scalar authorities still consume runtime checkpoints, and rollback, cleanup, and cursor-backed settlement are not yet driven by the structure. | Exact refusal timing and effect ordering. | effect-envelope phase remains active. | Supported dynamic work cannot be bounded before effects and the product contract must be reduced or separately changed. |
| ARCH-G13 | guarantee | Exclusive branches reserve the maximum reachable demand rather than the sum; sequential and repeated obligations preserve exact bounded demand and cannot add work after effects begin. | User decision; `ARCHITECTURE.md`. | Structural sequence, closed choice, bounded repeat, trigger/conditional, and ForwardPhase validation have differential apply/refresh demand parity; production physical lowering still consumes the legacy projection. | Behavioral parity and pre-effect refusal parity. | effect-envelope phase. | Runtime evidence proves branches are not closed/exclusive or promotion can create unplanned work. |
