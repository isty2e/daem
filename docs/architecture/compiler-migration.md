...
| effect-envelope phase | operation-authority phase facts and operation closure | passive typed structure, closed choices/triggers/conditionals, bounded demand frontiers, Apply/Refresh structural State Barrier lowering, and a Refresh-first exact-schedule runtime cursor pilot that consumes raw reserved StateDir and descendant physical authority without duplicate legacy semantic counters. Still required: Apply and Recovery runtime consumption, structural physical reservation without the legacy demand/path oracle, rollback/cleanup settlement, and removal of remaining flat demand and scalar runtime counters. | work cannot be bounded before effects or branch algebra is not closed |
