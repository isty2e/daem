...
| Recovery authority/fingerprint | `internal/operationplan`; recover execution/authority/replacement/cleanup tests including `TestCompiledActiveRecoveryFingerprintMatchesLegacyProjection`, `TestCompiledCleanupRecoveryFingerprintMatchesLegacyProjection`, and the active-journal retirement-structure ordering/failure-prefix tests | active and cleanup authority, drift, terminal classification, byte-identical active and cleanup-only operation projections, pure logical/physical domain-step compilation, and a deterministic zero-demand shadow of the post-cleanup active-journal transition tail; recover authority DTO omits Family/Containment | active and cleanup-only operation fingerprints compile in `operationplan`; workflow lowers paths while journal-owned action, statefile, cleanup-obligation, claim-transition, and retirement semantics remain owner-supplied or Effect-owned inputs. |
...
| effect-envelope phase | operation-authority phase facts and operation closure | passive typed structure, closed choices/triggers/conditionals, bounded demand frontiers, Apply/Refresh structural State Barrier lowering, a Refresh exact-schedule runtime cursor with structural physical reservation, a cleanup-only Recovery cursor pilot that consumes the exact journal-retirement and retained-authority-close schedule without moving journal semantics or changing RecoveryDir physical budgets, and a fixed-size active-Recovery post-cleanup journal-retirement cursor pilot with exact transition and failure prefixes while Journal retains transition ownership. Still required: Apply runtime consumption, broader active-Recovery removal/rollback/claim/outer-validation schedules, retained-authority close settlement, branch-selected Apply reservation reduction, rollback/compensation settlement, and removal of remaining flat demand and scalar runtime counters. | work cannot be bounded before effects or branch algebra is not closed |
...
| closeout phase | every prior artifact | provisional: surface compilation and production-consumer cutover, exact operation authority/fingerprint/revision-role compilation, pure workflow lowering, named State Barrier verification, Refresh cursor settlement, and the cleanup-only Recovery cursor pilot are present. Incomplete Apply cursor settlement, broader active-Recovery removal/rollback/claim/outer-validation and retained-authority-close settlement, remaining flat/scalar effect authority, rollback/compensation closure, and exact-path blocking classification still prevent closeout. | any duplicate source, bypass, parity drift, or unowned exception remains |
