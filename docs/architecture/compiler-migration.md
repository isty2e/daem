...
| operation-authority phase | operation/transition/compatibility rows | `internal/operationplan` fact/domain/revision compiler, exact apply/refresh/recover authority subprojections, adopt/import, init, lock, authoring, and unmanage domain/revision-role compilation, and exact apply/provider/remaining, refresh, active-recovery, and cleanup-only recovery fingerprints. The final caller census must classify every remaining mutating sibling path; probe remains observation-only unless separately authorized. | exact parity fails because facts are not actually shared |
...
| closeout phase | every prior artifact | provisional: surface compilation and production-consumer cutover, exact apply/refresh/recovery fingerprints, adopt/import, init, lock, authoring, and unmanage domain/revision-role compilation, and named State Barrier verification are present. Final sibling caller disposition, the count-only effect-envelope seed, and exact-path blocking classification still prevent closeout. | any duplicate source, bypass, parity drift, or unowned exception remains |
...
- Final caller census and canonical operation authority/revision/fingerprint
  cutover for any still-unmigrated mutating sibling path. Observation-only
  probe identity remains owner-local unless a
