...
| StateDir/file-set mechanics | `internal/effect/fileset` marker, rollback, recovery, and census tests; recoverygate StateDir identity tests | first-incarnation, budgets, residue, restore, limits | declaration/transaction remains the manifest/lockfile adapter. Mutating workflows validate recoverygate before `CommitFileSet`/`RecoverFileSet`. |
