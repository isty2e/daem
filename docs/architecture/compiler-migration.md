...
| ARCH-G17 | guarantee | Workflows gather boundary evidence, invoke pure owners, acquire capabilities, sequence effects, and project results; they do not retain competing surface, authority, fingerprint, or capacity grammars. | User decision; `ARCHITECTURE.md`. | MCP production callers, read-only location inventory, and selected apply/refresh/recover callers invoke `catalog.Product`, `operationplan`, and `recoverygate`. Instruction, Skill, Hook, HookAsset, and Extension views compile from owner-local snapshots; remaining adopt/diagnose/lock/readiness/effect consumers, workflow-local fingerprint DTOs, and several mutating-operation authority grammars remain migration work rather than accepted end state. | Behavioral parity. | surface-cutover through operation/effect closeout remains active. | A use case owns a genuinely operation-specific semantic contract not representable by the shared compiler. |
| ARCH-G18 | guarantee | Old implementation remains authoritative during shadow comparison. A caller flips only after exact required parity, and the old source is deleted after its last consumer moves. | User decision; architecture change rules. | Bounded MCP joins, read-only location inventory, apply/refresh/recover authority subprojections, the refresh operation fingerprint, and cleanup-only recovery fingerprint have flipped. Remaining non-MCP consumers, apply, active recovery, other mutating-operation compilers, and ordered effect settlement remain under the shadow-then-delete rule; final closeout has not occurred. | Exact or behavioral as classified below. | surface-schema phase through closeout phase. | A separately authorized migration requires staged dual-version support. |
...
| Non-MCP host-surface views | `internal/hostsurface/catalog`: managed-path, Hook/HookAsset, and Extension owner-parity, target/scope lookup, and negative-seed tests; `internal/realization/profile`: managed-path/delegated-route/order/HookAsset snapshot tests; `internal/topology/extension`: carrier namespace coverage; `internal/workflow/list`: exact location inventory/output tests | exact Instruction/Skill placement, admission/default, discovery/runtime, and route facets; Hook aggregate placement/codec/routes; shared HookAsset physical placement; Extension carrier/source/namespace/delegated-route/order facets; stable owner and SurfaceID ordering; defensive copies; list inventory parity | read-only list inventory consumes compiled views. Adopt/diagnose/lock/readiness/effect consumers remain on owner-local projections until bounded differential cutover. Skill compatibility policy remains a separate Supply oracle. |
...
| Recovery authority/fingerprint | `internal/operationplan`; recover execution/authority/replacement/cleanup tests including `TestCompiledCleanupRecoveryFingerprintMatchesLegacyProjection` | active and cleanup authority, drift, terminal classification, byte-identical cleanup-only operation projection; recover authority DTO omits Family/Containment | recover authority and cleanup-only operation fingerprints compile in `operationplan`; the active recovery operation serializer remains workflow-local and must preserve journal-owned facts as opaque inputs. |
...
| surface-cutover phase | surface-schema phase views and consumer census | bounded cutover, deletion map, one source per migrated facet. MCP waves 1–4 use compiled target/scope, probe/provider, adopt-local extractor, subject, readiness/observe, and owner-order views while preserving Pi importability and realization/lock ownership. Non-MCP wave 1 moves read-only list inventory for managed paths, Hook/HookAsset, and Extension routes while leaving manifest placement selection owner-local. Remaining caller groups each require differential parity before their old join is removed. | any consumer requires different semantics or facade becomes authority |
| operation-authority phase | operation/transition/compatibility rows | `internal/operationplan` fact/domain/revision compiler, exact apply/refresh/recover authority subprojections, exact refresh operation fingerprint, and exact cleanup-only recovery fingerprint. Still required: apply and active-recovery serializers, adopt safety compilation, authoring/unmanage/init/lock and every other mutating sibling path; probe remains observation-only unless separately authorized. | exact parity fails because facts are not actually shared |
...
| readiness phase | active surface views and operation vocabulary | observation shell and pure readiness middle-end. `Assess` and `AssessOutputInventory` remain the observation sequencers; `assembleAssessment` / `planOutputInventory` / `planExtensionOrderDecisions` perform no I/O; Skill support uses owner-local `profile.TargetSupports`; compiled `LookupMCPBySubject` remains the active MCP cell join. New non-MCP catalog views remain shadow-only and are not readiness inputs. `operationplan` stays in apply/refresh/recover. Observation order and codec `MaximumDocumentBytes` bounds stay exact. | observation order/evidence semantics cannot remain exact |
...
| closeout phase | every prior artifact | provisional: non-MCP compilation, the list-inventory cutover, and named State Barrier verification are present, but remaining production surface joins, workflow-local operation/fingerprint compilers, the count-only effect-envelope seed, and exact-path blocking classification still prevent closeout | any duplicate source, bypass, parity drift, or unowned exception remains |
...
Status: provisional census for the bounded MCP cell join, non-MCP shadow
catalog and list-inventory cutover, apply/refresh/recover authority and
...
program closeout. Remaining Instruction, Skill, Hook, HookAsset, and Extension
consumer joins; mutating-operation compilers and fingerprint serializers;
ordered effect settlement; and semantic guard replacement remain required work.
