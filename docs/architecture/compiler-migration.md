...
| effect-envelope phase | operation-authority phase facts and operation closure | passive typed structure, closed choices/triggers/conditionals, bounded demand frontiers, Apply/Refresh structural State Barrier lowering, a Refresh exact-schedule runtime cursor with structural physical reservation, and a cleanup-only Recovery cursor pilot that consumes the exact journal-retirement and retained-authority-close schedule without moving journal semantics or changing RecoveryDir physical budgets. Still required: Apply and active-Recovery runtime consumption, branch-selected Apply reservation reduction, rollback/compensation settlement, and removal of remaining flat demand and scalar runtime counters. | work cannot be bounded before effects or branch algebra is not closed |
