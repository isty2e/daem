...
func TestFailureDetailCatalogCoversEveryWorkflowReason(t *testing.T) {
	tests := []struct {
		...
		{name: "interrupted apply", result: failedResult(ReasonInterruptedApply), want: "interrupted apply journal is present; run daem recover first"},
		{name: "interrupted apply with file-set fence", result: failedResult(ReasonInterruptedApplyFileSetFence), want: "interrupted apply journal is present; run daem recover first; the file-set fence remains after recover"},
		{name: "journal cleanup", result: failedResult(ReasonJournalCleanupIncomplete), want: "journal cleanup is incomplete; run daem recover first"},
		{name: "journal cleanup with file-set fence", result: failedResult(ReasonJournalCleanupFileSetFence), want: "journal cleanup is incomplete; run daem recover first; the file-set fence remains after recover"},
		{name: "interrupted file-set transaction", result: failedResult(ReasonInterruptedFileSetTransaction), want: "an interrupted file-set transaction requires its owning workflow to recover it"},
		{name: "invalid file-set evidence", result: failedResult(ReasonFileSetEvidenceInvalid), want: "file-set transaction evidence is incomplete or invalid"},
		{name: "abandoned file-set residue", result: failedResult(ReasonAbandonedFileSetResidue), want: "abandoned file-set residue remains; current daem cannot recover markerless residue"},
		{name: "file-set census limit", result: failedResult(ReasonFileSetFenceCensusLimit), want: "the bounded file-set fence census could not prove the fence clean"},
		{name: "file-set access unprovable", result: failedResult(ReasonFileSetAccessUnprovable), want: "file-set state directory access or identity could not be proven"},
