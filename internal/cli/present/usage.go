...
func helpPages(context UsageContext) map[string]helpPage {
	...
	pages["refresh extension"] = leaf("daem refresh extension", "daem refresh extension <id> [--manifest <path>] [--target <target>] [--scope <scope>] [options]", "refresh one exact declared and locked extension relation",
		[]helpRow{{"<id>", "Exact declared extension id; no bulk or wildcard selection."}},
		[]helpRow{workspace, {"--target <target>", "Require the selected extension to use this exact target."}, {"--scope <scope>", "Require the selected extension to use this exact scope."}, dryRun, yes, jsonOutput, verbose},
		[]helpRow{{"", "Refresh never rewrites the manifest, lockfile, or managed ownership."}, {"", "Bare execution requires terminal stdin/stdout/stderr and confirms after complete stdout disclosure."}, {"", "Non-interactive execution requires --yes. JSON mutation writes its authorization disclosure to stderr and one final result document to stdout."}, {"", "A required observer must prove the exact relation before and after execution; an admitted no-observer route reports attempted_unverified."}},
