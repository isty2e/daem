...
type RemovalNamespaceAuthority struct {
	...
	parent           RootProvenance
	retainedAncestor RootProvenance
	missingSuffix    string
	names            mutationfs.LogicalRemovalNames
}

// NewExistingParentAuthority binds a residue to the exact parent incarnation.
// Reconciliation requires that exact parent to remain observable at its
// captured path because absence cannot distinguish unlink from relocation.
...
func (authority RemovalNamespaceAuthority) ParentProvenance() (RootProvenance, bool) {
...
func (authority RemovalNamespaceAuthority) RetainedAncestorProvenance() (RootProvenance, bool) {
	return authority.retainedAncestor, authority.variant == RemovalNamespaceInitiallyAbsentParent
}
...
func (authority RemovalNamespaceAuthority) validate() error {
	...
	case RemovalNamespaceExistingParent:
		...
		if authority.retainedAncestor != (RootProvenance{}) || authority.missingSuffix != "" {
			return fmt.Errorf("existing parent authority carries initially-absent parent facts")
		}
	case RemovalNamespaceInitiallyAbsentParent:
		...
		if authority.parent != (RootProvenance{}) {
...
// MaximumRemovalStatesPerIntent admits at most the complete before and
// expected-after whole-path states reachable in one operation.
const MaximumRemovalStatesPerIntent = 2
...
func validateRemovalExistingFacts(kind, contentHash, backupPath, linkTarget string, mode *PermissionMode) error {
	if mode != nil {
		if err := validatePermissionMode(*mode, "removal state path_mode"); err != nil {
			return err
		}
	}
	switch kind {
...
// AdmitsEntry reports whether fresh whole-path facts match this exact state.
func (state RemovalState) AdmitsEntry(observation RemovalResidueEntryObservation) bool {
	return observation.status == RemovalResidueEntryPresent &&
		removalStateAdmitsEntry(state, observation)
}
...
func NewRemovalDemand(scope target.Scope, destination output.Destination, states []RemovalState) (RemovalDemand, error) {
	...
	if len(states) > MaximumRemovalStatesPerIntent {
		return RemovalDemand{}, fmt.Errorf(
			"removal demand state count %d exceeds maximum %d",
			len(states),
			MaximumRemovalStatesPerIntent,
		)
	}
	copy := append([]RemovalState(nil), states...)
	seenStates := make(map[string]struct{}, len(copy))
...
func (demand RemovalDemand) equal(other RemovalDemand) bool {
	if demand.scope != other.scope || demand.destination != other.destination || len(demand.states) != len(other.states) {
		return false
	}
	for index := range demand.states {
		if !demand.states[index].equal(other.states[index]) {
			return false
		}
	}
	return true
...
func (demand RemovalDemand) clone() RemovalDemand {
	return RemovalDemand{
		scope:       demand.scope,
		destination: demand.destination,
		states:      slices.Clone(demand.states),
	}
}
...
const (
	...
	RemovalCleanupReasonActionFailed         RemovalCleanupReason = "cleanup_action_failed"
)
...
func (obligation RemovalCleanupObligation) Scope() target.Scope { return obligation.intent.Scope() }
...
func (obligation RemovalCleanupObligation) Destination() output.Destination {
	return obligation.intent.Destination()
}
...
func (obligation RemovalCleanupObligation) Names() mutationfs.LogicalRemovalNames {
	return obligation.intent.Namespace().Names()
}
...
func (obligation RemovalCleanupObligation) Equal(other RemovalCleanupObligation) bool {
	return obligation.intent.equal(other.intent) &&
...
func NewPendingRemovalCleanupObligations(
	...
	if len(intents) > MaximumRemovalIntents {
		return nil, fmt.Errorf(
			"removal cleanup obligation count %d exceeds operation maximum %d",
			len(intents),
...
func NewRemovalResidueEntryObservation(
	...
	case RemovalResidueEntryAbsent:
		...
		if detail != "" {
			return RemovalResidueEntryObservation{}, fmt.Errorf("absent residue observation must not contain detail")
		}
	case RemovalResidueEntryPresent:
		...
		if detail != "" {
			return RemovalResidueEntryObservation{}, fmt.Errorf("present residue observation must not contain detail")
		}
	case RemovalResidueEntryUnsupported, RemovalResidueEntryUnavailable:
		if kind != "" || contentHash != "" || pathMode != nil || linkTarget != "" {
			return RemovalResidueEntryObservation{}, fmt.Errorf("%s residue observation carries entry facts", status)
		}
		if strings.TrimSpace(detail) == "" {
...
func (intent RemovalIntent) AdmitsEntry(observation RemovalResidueEntryObservation) bool {
	...
	for _, state := range intent.demand.states {
...
func NewRemovalIntent(demand RemovalDemand, namespace RemovalNamespaceAuthority) (RemovalIntent, error) {
	canonicalDemand, err := NewRemovalDemand(demand.scope, demand.destination, demand.states)
	if err != nil {
...
func (intent RemovalIntent) Demand() RemovalDemand { return intent.demand.clone() }

// Scope returns the intent relation scope.
func (intent RemovalIntent) Scope() target.Scope { return intent.demand.Scope() }

// Destination returns the intent relation destination.
func (intent RemovalIntent) Destination() output.Destination { return intent.demand.Destination() }
...
func (intent RemovalIntent) States() []RemovalState { return intent.demand.States() }
...
func (intent RemovalIntent) AdmitsState(state RemovalState) bool {
	for _, candidate := range intent.demand.states {
...
func (intent RemovalIntent) validate() error {
	if err := intent.demand.validate(); err != nil {
		return err
	}
	return intent.namespace.validate()
}

func (intent RemovalIntent) equal(other RemovalIntent) bool {
	return intent.demand.equal(other.demand) && intent.namespace.equal(other.namespace)
}

func (intent RemovalIntent) clone() RemovalIntent {
	return RemovalIntent{demand: intent.demand.clone(), namespace: intent.namespace}
}
