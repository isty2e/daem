...
func (persisted *recoveryJournalDTO) UnmarshalJSON(content []byte) error {
	...
	if err := requireRecoveryJSONArrayMaximum(
		removalIntents,
		"recovery journal removal_intents",
...
func validateRecoveryJournalRelationships(journal recoveryJournal) error {
	...
	if err := validatePersistedRemovalIntentCoverage(journal.Entries, removalIntents); err != nil {
		return fmt.Errorf("validate recovery removal intent coverage: %w", err)
	}
