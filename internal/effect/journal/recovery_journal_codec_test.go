...
func TestRecoveryJournalRejectsVersionSix(t *testing.T) {
	...
	content = bytes.Replace(content, []byte(`"version": 12`), []byte(`"version": 6`), 1)
...
func TestRecoveryJournalRejectsReusableMountVersionTen(t *testing.T) {
	...
	content = bytes.Replace(content, []byte(`"version": 12`), []byte(`"version": 10`), 1)
	directory, err := filepath.EvalSymlinks(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoveryJournalClassifiesFutureVersionBeforeStrictSchema(t *testing.T) {
	...
	content = bytes.Replace(content, []byte(`"version": 12`), []byte(`"version": 13, "future": true`), 1)
	directory, err := filepath.EvalSymlinks(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
