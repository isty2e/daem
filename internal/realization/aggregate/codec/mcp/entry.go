...
func decodeOpenCodeProjectMCPServerEntry(raw json.RawMessage, serverID string) (OpenCodeMCPServerEntry, error) {
	...
	sort.Strings(fieldNames)
	for _, fieldName := range fieldNames {
		if _, ok := allowedOpenCodeProjectMCPManagedFields[fieldName]; !ok {
			return OpenCodeMCPServerEntry{}, newMCPProjectionError(
				MCPProjectionReasonUnsupportedManagedField,
				subject,
				fmt.Sprintf("unsupported managed MCP server field %q", fieldName),
