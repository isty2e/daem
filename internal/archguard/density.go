...
var packageDensityAdmissions = map[string]densityReviewAdmission{
	...
	"internal/effect/execute": {
		...
		reason:              "at 22 production files, mutation, per-effect visibility authority, verification, rollback, active recovery, recovery-root-only journal cleanup, and state commit have distinct authorities while sharing one ordered effect boundary",
