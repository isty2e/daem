...
var packageDensityAdmissions = map[string]densityReviewAdmission{
	...
	"internal/effect/execute": {
		...
		reason:              "at 24 production files, mutation, per-effect visibility authority, verification, rollback, active recovery, exact rooted-removal authority, recovery-root-only journal cleanup, and state commit have distinct authorities while sharing one ordered effect boundary",
		naturalSplit:        "separate effect phases into child packages",
		alternativeRejected: "phase packages would expose private transaction state, duplicate the apply and recovery visibility gate, and weaken the single ordered effect boundary",
	},
	"internal/effect/journal": {
		reviewedValue:       28,
		owner:               "journal persistence transaction",
		reason:              "at 28 production files, capture, typed removal intent authority, provisional ownership intent, CAS-bound exact promotion, backup identity, closed recovery-root inventory, operation-local journal-directory authority, retirement selection, effect sequencing, no-effect verification, and active lifecycle evolve atomically under one private wire schema and physical authority boundary",
		naturalSplit:        "move recovery-root inventory into an observation child package",
		alternativeRejected: "the child would need parent-owned journal decoding while the parent consumed its classification and promotion evidence, forcing a cycle or exporting private wire and authority facts; wire-neutral recovery and retirement algebras are already isolated",
	},
	...
	"internal/effect/storage/commit": {
		...
		reason:              "at 27 production files, commit phases, no-follow entry and bounded tree snapshots, mount-confined metadata preflight, staged ancestor identity and no-replace publication, exact same-parent rename and cleanup, identity revalidation, visibility, residue, and rollback share one platform-selected transaction",
		naturalSplit:        "move stable read and traversal observations into a child adapter package",
		alternativeRejected: "the child would duplicate platform identity, descriptor-anchor, mount-token, and traversal-budget logic or export transaction internals shared with guarded mutation and rollback",
	},
