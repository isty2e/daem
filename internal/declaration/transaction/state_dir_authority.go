...
// StateDir namespace. A planning-time absence remains absent until the owning
// workflow explicitly accepts a directory created by its own effect.
...
func CaptureStateDirAuthority(ctx context.Context, stateDir string) (StateDirAuthority, error) {
	...
	namespacePath, namespaceIdentity, namespaceMount, namespaceIncarnation, err := captureStateDirNamespace(ctx, canonical)
	if err != nil {
		return StateDirAuthority{}, err
	}
	present, identity, mount, incarnation, err := observeCurrentStateDir(ctx, canonical)
	if err != nil {
		return StateDirAuthority{}, err
	}
...
func (authority StateDirAuthority) Validate(ctx context.Context) error {
	...
	if err := validateStateDirNamespace(ctx, snapshot); err != nil {
		return err
	}
	present, observedIdentity, observedMount, observedIncarnation, err := observeCurrentStateDir(ctx, snapshot.path)
	if err != nil {
		return err
	}
...
func validateStateDirNamespace(ctx context.Context, snapshot stateDirAuthoritySnapshot) error {
...
// AcceptCurrentIncarnation binds the first StateDir directory created by an
// owning workflow effect after a planning-time absence.
func (authority StateDirAuthority) AcceptCurrentIncarnation(ctx context.Context) error {
	snapshot, ok := authority.snapshot()
	if !ok {
		return fmt.Errorf("file-set state directory authority is uninitialized")
	}
